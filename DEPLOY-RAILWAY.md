...
**`gyp ERR! find Python` or `no prebuilt binaries found`** — the builder picked a
Node version that `better-sqlite3` has no prebuilt binary for, so it tried to
compile from source. `nixpacks.toml`, `.nvmrc` and the `engines` field all pin
Node 22 to prevent this. If you bump Node later, check `better-sqlite3` ships a
binary for that major first.
